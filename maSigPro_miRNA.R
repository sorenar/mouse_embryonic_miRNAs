...
data.counts = read.csv("CPMs.TMMnormalized.csv", row.names="Symbol")
# remove the spike ins
data.counts = data.counts[-c(783:785),colnames(data.counts) %in% edesignfile$SL.ID]
...
save(datat,file = "miRNA_maSigPro_TMM.RData")

get<-get.siggenes(datat,rsq=0.7, vars="all") # get the list of significant genes
x <- see.genes(get$sig.genes,k=16)$cut
y <- data.frame(x)
write.table(y,"miRNA_clusters_16.txt", sep="\t", quote=FALSE, col.names=FALSE)
...
samples <- read.csv("samples.all.modified.csv", header=TRUE, stringsAsFactors = F)
tpms <- read.csv("CPMs.TMMnormalized.modified.csv", header=T, row.names = "Symbol")
tpms <- tpms + 0.1
...
for (clust in 1:16){
  cluster <- rownames(y)[y$x == clust]#cluster = miRNAclusters[miRNAclusters$cluster.number == clust,1]#
  cluster.data <- tpms[cluster,]
  #write.table(cluster3.data,"cluster3.txt", sep="\t", quote=FALSE, col.names=T)
  ...
  clust.name <- plot_num <- paste("cluster",clust,": ",length(cluster), " miRNAs",sep = "")
  ...
  p <- ggplot(df, aes(x=time, y=medians, group=tissue, color = tissue)) + 
    ...
    labs(y="Median profile (CPM)",x="Developmental stage",title = clust.name)
...
grid.arrange(fig1,fig2,fig3,fig4,fig5,fig6,fig7,fig8,fig9,fig10,fig11,fig12,
             ...
             ncol=4)
dev.print(pdf, paste('miRNA_clusters_',16,'.pdf'),width = 50, height = 30)
